...
`另外，SGD最开始指的是单样本更新，这是我们这里提到的，也是最原始的，后面逐渐被大家理解为是下面的MBGD，所以，当别人说SGD,也要考虑是不是说MBGD。`
### 小批量梯度下降法(MBGD)
...
$$
\begin{split}
&1.Randomly\;Dataset\;Shuffle\\
&2.Say\;b=10,n=1000\\
&Repeat \; \; \{ \\
&\;\;\;\;\;\;\; for \; i=1,11,21,31,...,991 \{ \\
&\;\;\;\;\;\;\;\;\;\;\;\;\;\; w_{t+1}=w_t-\alpha*(\frac{w_t}{5}*\sum_{k=i}^{i+9}(w_tx_i+b_t-y_i)) \\
&\;\;\;\;\;\;\;\;\;\;\;\;\;\; b_{t+1}=b_t-\alpha*(\frac{1}{5}*\sum_{k=i}^{i+9}(w_tx_i+b_t-y_i)) \\
&\;\;\;\;\;\;\;\}\\
&\}
\end{split}
$$

通过这种批采样方法来更新$w_{t+1}$可以达到在迭代速度和采样噪声影响上取得折衷的效果。另外,顺便提一下，伪代码中$Repeat$一次，就机器学习里面常说的`epoch`，即遍历一次数据集，其实1次$epoch$却更新了$\frac{n}{b}$次。
### 梯度下降分类总结
...
逆是否存在？实质是线性变换是否会压缩到更低空间，使得原来的基向量张成的空间围成的体积，也就是`矩阵行列式为零`，即$|X|=0$。

###梯度下降与正规方程总结

##鞍点
鞍点周围的梯度为零，也可以说鞍点通常被相同误差值的平面所包围(这个平面又叫Plateaus，PLateaus是梯度接近于0的平缓区域，会降低神经网络地学习速度，在高维的情形，这个鞍点附近的平坦区域范围可能非常大，这使得SGD算法很难脱离区域，可能会长时间卡在该点附近)。在鞍点数目极大的时候，这个问题会变得非常严重。高维非凸优化问题之所以困难，是因为高维参数空间大量的鞍点存在。


##loss的分类
### Mean Squared Error
$$loss = \sum[y-(xw+b)]^2$$
$$L2 -norm =\Vert y-(xw+b)\Vert_2$$
$$loss = norm(y-(wx+b))^2$$
#### Derivative
